...
async function build({ config = {} }) {
    ...
    await fs.rename(`${pkg}/administrator/components/com_pages/pages.xml`, `${pkg}/pages.xml`);
...
async function buildExtensions({ config = {} }) {
    ...
    const extensionFolders = await fs.readdir(contribFolder, { withFileTypes: true });

    await Promise.all(extensionFolders.filter(dirent => dirent.isDirectory()).map((dirent) => {
        const extensionFolder = dirent.name;

        mason.log.debug(`Building extension: ${extensionFolder}…`);

        return mason.fs.archiveDirectory(path.join(contribFolder, extensionFolder), `${config.location}/com_pages-extension-${extensionFolder}.zip`);
    }));
}
...
async function bundle({ config = {} }) {
    ...
    if (!config.githubToken) {
        throw new Error('Github token is required for bundle task to download joomlatools-extension-installer');
    }
    ...
    await mason.fs.archiveDirectory(installer, `${pagesRoot}/com_pages${version ? '_v'+version[1] : '_bundle'}.zip`);
